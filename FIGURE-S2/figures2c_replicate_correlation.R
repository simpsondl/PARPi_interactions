...
phenotypes2 <- phenotypes %>% group_by(GuideCombinationID) %>%
  summarise(Category = unique(Category),
            Gamma.R1 = mean(Gamma.R1),
            Gamma.R2 = mean(Gamma.R2),
            Tau.R1 = mean(Tau.R1),
            Tau.R2 = mean(Tau.R2))
rho2 <- rho %>% group_by(GuideCombinationID) %>%
  summarise(Category = unique(Category),
            Rho.R1 = mean(Rho.R1),
            Rho.R2 = mean(Rho.R2))
...
phenotypes2 <- inner_join(phenotypes2,
                          rho2,
                          by = c("GuideCombinationID", "Category"))
...
g <- ggplot(tmp.df,aes(Gamma.R1, Gamma.R2, color = Category)) + 
        ...
        scale_y_continuous(limits = c(-.75, .25), breaks = c(-.75, -.5, -.25, 0, .25)) +
        theme_bw() + 
        nolabel_theme +
        nolabel_axes 
...
# Save plots
ggsave("../FIGURES/FIGURE-S2/figure_s2c_gammaphenotype_replicate_correlation.png", 
       gh, device = "png", width = 2.5, height = 2.5, dpi = 300)
ggsave("../FIGURES/FIGURE-S2/figure_s2c_gammaphenotype_replicate_correlation_labels.svg", 
       gl, device = "svg", width = 6, height = 6)
ggsave("../FIGURES/FIGURE-S2/figure_s2c_tauphenotype_replicate_correlation.png", 
       th, device = "png", width = 2.5, height = 2.5, dpi = 300)
ggsave("../FIGURES/FIGURE-S2/figure_s2c_tauphenotype_replicate_correlation_labels.svg", 
       tl, device = "svg", width = 6, height = 6)
ggsave("../FIGURES/FIGURE-S2/figure_s2c_rhophenotype_replicate_correlation.png", 
       rh, device = "png", width = 2.5, height = 2.5, dpi = 300)
ggsave("../FIGURES/FIGURE-S2/figure_s2c_rhophenotype_replicate_correlation_labels.svg", 
